...
import com.thalasoft.post.assembler.PostModelAssemblerSupport;
...
public class PostController {
    ...
    private final PostModelAssemblerSupport postModelAssemblerSupport;
    private final PagedResourcesAssembler<PostModel> pagedResourcesAssembler;
    private Set<String> nonSortableColumns;

    public PostController(ModelService modelService, PostService postService,
            PostModelAssemblerSupport postModelAssembler, PagedResourcesAssembler<PostModel> pagedResourcesAssembler) {
        this.modelService = modelService;
        this.postService = postService;
        this.postModelAssemblerSupport = postModelAssembler;
    ...
    //     modelService.addSortToPageable(pageable, sort);
    //     Page<Post> posts = postService.findAll(pageable);
    //     PagedModel<PostModel> pagedPostModels = pagedResourcesAssembler.toModel(posts, postModelAssemblerSupport);
    ...
    public ResponseEntity<CollectionModel<PostModel>> hateoasFindAll() {
        return ResponseEntity.ok().body(postModelAssemblerSupport.toCollectionModel(postService.findAll()));
    }

    @GetMapping("/{id}")
    public ResponseEntity<PostModel> findById(@PathVariable final Long id) {
        return Optional.ofNullable(postModelAssemblerSupport.toModel(postService.findById(id)))
    ...
    public ResponseEntity<PostModel> save(@RequestBody @Valid final PostModel postModel, UriComponentsBuilder builder) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).headers(responseHeaders)
                .body(postModelAssemblerSupport.toModel(post));
    }
    ...
    public ResponseEntity<PostModel> update(@PathVariable final Long id, @RequestBody @Valid final PostModel postModel,
        ...
        PostModel updatedPostModel = postModelAssemblerSupport.toModel(post);
        URI location = builder.path(RESTUtils.SLASH + RESTUtils.POSTS + RESTUtils.SLASH + "{id}")
                .buildAndExpand(post.getId()).toUri();
        HttpHeaders responseHeaders = new HttpHeaders();
        responseHeaders.setLocation(location);
    ...
    public ResponseEntity<PostModel> partialUpdate(@PathVariable Long id, @Valid @RequestBody PostModel postModel,
        ...
        PostModel updatedUserModel = postModelAssemblerSupport.toModel(post);
        URI location = builder.path(RESTUtils.SLASH + RESTUtils.POSTS + RESTUtils.SLASH + "{id}")
                .buildAndExpand(post.getId()).toUri();
        HttpHeaders responseHeaders = new HttpHeaders();
        responseHeaders.setLocation(location);
